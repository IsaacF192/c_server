...
#include <filesystem>  // Make sure this is at the top of your file
...
HttpRequest req(request_data);  // Safely construct the request
   ...
   // Canonicalize root directory (only once)
std::filesystem::path safe_root = std::filesystem::canonical(ROOT_DIR);

// Build requested path
std::string cleaned_path = (req.path == "/" ? "index.html" : req.path);

// Remove any leading slash so it's treated as relative
if (!cleaned_path.empty() && cleaned_path[0] == '/') {
    cleaned_path.erase(0, 1);
}

std::filesystem::path requested_path = cleaned_path;
// Compose full file path inside root
std::filesystem::path full_path = safe_root / requested_path;

std::string response;

logger.log(Logger::INFO, "Decoded path: " + req.path);
logger.log(Logger::INFO, "Safe root: " + safe_root.string());
logger.log(Logger::INFO, "Requested path: " + requested_path.string());
logger.log(Logger::INFO, "Full path (pre-canonical): " + full_path.string());
...
try {
    ...
    logger.log(Logger::INFO, "Trying to canonicalize: " + full_path.string());

} catch (const std::filesystem::filesystem_error& e) {
    std::string err = e.what();
    logger.log(Logger::ERROR, "Filesystem error: " + err);

    // If user requested a path containing ".." and canonicalization failed
    if (cleaned_path.find("..") != std::string::npos) {
        logger.log(Logger::ERROR, " Blocked suspected path traversal based on '..' in path: " + cleaned_path);
        HttpResponse res(403, "<h1>403 Forbidden</h1>");
        response = res.to_string();
        send(client_fd, response.c_str(), response.size(), 0);
        close(client_fd);
